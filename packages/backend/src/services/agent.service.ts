...
async function executeTool(
  ...
  switch (name) {
    case 'get_call_tree': {
      ...
      const lines = buildCallTreeText(state.callTree, 0, [], Infinity, Infinity, revertPathIds, revertOriginId);
      return lines.join('\n');
    }
    ...
    case 'get_call_subtree': {
      ...
      const lines = buildCallTreeText(node, 0, [], Infinity, Infinity, new Set(), '');
      return lines.join('\n');
    }
